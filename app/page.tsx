...
export default function Home() {
  ...
  // Component for the text content overlay
  const TextContent = ({ sectionIndex }: { sectionIndex: number }) => {
    if (sectionIndex === 0) {
      // Home section - no overlay content, just empty space
      return null;
    } else {
      // Content sections
  ...
  return (
    ...
    >
      <div className="hidden lg:flex gap-[20px] content-start items-center justify-start -ml-20">
              ...
              {[0, 1, 2, 3, 4].map((sectionIndex) => (
                <div key={sectionIndex} className="w-full h-full snap-start relative">
                  <TextContent sectionIndex={sectionIndex} />
                    ...
                    {/* Fixed caption below image - positioned outside scroll area */}
        ...
        {/* Decorative lines positioned right of the image container */}
        <Image
          src="/lines.svg"
